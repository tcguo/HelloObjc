...
@interface YTExpandableCollectionView () <UIGestureRecognizerDelegate, UICollectionViewDataSource>
...
@implementation YTExpandableCollectionView
            ...
            [UIView animateWithDuration:0.2 animations:^{
                [self performBatchUpdates:^{
                    ...
                    if (willOpen) {
                        ...
                        NSInteger totaloffsetXX = tappedSection * (layout.itemSize.width +12);
                        
                        [self setContentOffset:CGPointMake(totaloffsetXX, 0) animated:YES];
                        return;
                        ...
                        CGFloat firstItemTop = firstItem.frame.origin.x;
                        CGFloat lastItemBottom = lastItem.frame.origin.x + lastItem.frame.size.width;
                        CGFloat height = self.bounds.size.width;
                
                        NSIndexPath *firstIndexPath = [NSIndexPath indexPathForItem:0 inSection:tappedCellPath.section];
                        [self setContentOffset:CGPointMake(firstItemTop, 0) animated:YES];
                        
                        return;
                        
                        if (lastItemBottom - self.contentOffset.x > height) {
                            if (lastItemBottom - firstItemTop > height) {
                                ...
                                [self setContentOffset:CGPointMake(firstItemTop, 0) animated:YES];
                                //                                NSIndexPath *firstIndexPath = [NSIndexPath indexPathForItem:0 inSection:tappedCellPath.section];
                                //                                [self scrollToItemAtIndexPath:firstIndexPath atScrollPosition:UICollectionViewScrollPositionLeft animated:YES];
                            } else {
                                [self setContentOffset:CGPointMake(lastItemBottom - height, 0.) animated:YES];
                                //                                NSIndexPath *lastIndexPath = [NSIndexPath indexPathForItem:0 inSection:tappedCellPath.section];
                                //                                [self scrollToItemAtIndexPath:lastIndexPath atScrollPosition:UICollectionViewScrollPositionLeft animated:YES];
                            }
